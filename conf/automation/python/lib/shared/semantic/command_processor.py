...
from shared.semantic.model.semantic_model import SemanticModel
...
class CommandProcessor:
    ...
    def getItemsByType(self,parent,type):
        ...
        if parent.getItem().getType() == "Group":
            #logger.info(" => {} {}".format(parent.getItem().getName(),parent.getItem().getType()))
            semantic_items = parent.getChildren()
            for semantic_item in semantic_items:
                #logger.info(" => {}".format(semantic_item))
        ...
        if semantic_item.getItem().getType() == "Group":
            for _item in semantic_item.getItem().getMembers():
                try:
                    ...
                    #logger.info("item {} not found".format(_item.getName()))
                    pass
    ...
    def checkPoints(self,action,unprocessed_search,items_to_skip):
        ...
        for location_match in action.location_matches:
            ...
            if self.debug:
                logger.info("  location: '{}' => '{}'".format(location_match.semantic_item.getItem().getName(),location_match.all_matches))
            ...
            for equipment in equipments:
                ...
                for match in matches:
                    if self.debug:
                        logger.info("      item: '{}' => '{}'".format(match.semantic_item.getItem().getName(),match.all_matches))

                    key = "{},{}".format(",".join(location_match.all_matches),",".join(match.all_matches))
    ...
    def fillPointFallbacks(self,actions):
        ...
        for action in reversed(actions):
            ...
            if len(action.points) == 0:
                if last_action != None:
                    self.checkPoints(action,"{} {}".format(" ".join(last_action.point_search_terms),action.unprocessed_search),last_action.points)
            else:
                last_action = action
    ...
    def checkCommand(self,action):
        ...
        #logger.info(">>>> {}".format(main_property))
            ...
            for cmd_config in SemanticConfig["commands"][cmd_type]:
                for search in cmd_config["search"]:
                    if cmd_config["value"] == "REGEX":
                        #logger.info("{} {}".format(search[1:-1],action.unprocessed_search))
        ...
        for action in actions:
            for point in action.points:
                ...
                if item_name in processed_items \
                    ...
                    or ( "tags" in action.cmd.cmd_config and len(list(filter(lambda x: x in action.cmd.cmd_config["tags"], point.item.getTags())))==0  ):
                    # TODO debug if all cases makes sence
                    #logger.info(">>>>skip {} {}".format(item_name,action.cmd))
                    continue
                processed_items[item_name] = True
                action.item_actions.append(ItemAction(point.item,action.cmd.cmd_type,action.cmd.cmd_value))
        #logger.info("{}".format(processed_items.keys()))
    ...
    def process(self,voice_command, fallback_location_name):
        ...
        for search in self.full_phrase_terms:
            if search == voice_command:
                ...
                #logger.info("process - search: {}".format(search))
                break
    ...
    def getAnswer(self,semantic_item):
        ...
        for tag in semantic_item.getItem().getTags():
            ...
            if java.instanceof(semantic_item.getItem(), Java_NumberItem):
            ...
            for action in actions:
                for item_action in action.item_actions:
                    ...
                    if item_action.cmd_type == "READ":
                        ...
                        if not dry_run:
                            Registry.getItem(item_action.item.getName()).sendCommandIfDifferent(item_action.cmd_value)
